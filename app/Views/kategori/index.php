...
<body>
            ...
            <div class="container-fluid">
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form action="<?= base_url('master_kategori/add_kategori') ?>" method="post">
                                    <div class="mb-3">
                                        <label for="accUsername" class="form-label">Kode Kategori</label>
                                        <input type="text" class="form-control" id="kalKode" name="kalKode"
                                            placeholder="Masukkan Kode" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="accName" class="form-label">Nama Lengkap Kategori</label>
                                        <input type="text" class="form-control" id="kalNama" name="kalNama"
                                            placeholder="Masukkan Nama Lengkap" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="accPasswd" class="form-label">Nama Pendek Kategori</label>
                                        <input type="text" class="form-control" id="kalSingkatan" name="kalSingkatan"
                                            placeholder="Masukkan Nama Singkatan" required>
                                    </div>
                ...
                <div class="modal fade" id="bs-example-modal-lg-edit" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-lg">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h4 class="modal-title" id="myLargeModalLabelEdit">Ubah Kategori</h4>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"
                                    aria-hidden="true"></button>
                            </div>
                            <div class="modal-body">
                                <form action="<?= base_url('master_kategori/edit_kategori') ?>" method="post">
                                    <input type="hidden" id="kalKodeEdit" name="kalKodeEdit">
                                    <div class="mb-3">
                                        <label for="accNameEdit" class="form-label">Nama Lengkap Kategori</label>
                                        <input type="text" class="form-control" id="kalNamaEdit" name="kalNamaEdit"
                                            placeholder="Masukkan Nama" required>
                                    </div>
                                    <div class="mb-3">
                                        <label for="accNameEdit" class="form-label">Nama Pendek Kategori</label>
                                        <input type="text" class="form-control" id="kalSingkatanEdit" name="kalSingkatanEdit"
                                            placeholder="Masukkan Nama" required>
                                    </div>
    ...
    <script>
        $(document).ready(function () {
            $('#myTable').DataTable({
                ajax: '<?php echo base_url(); ?>master_kategori/datatable',
                processing: true,
                serverSide: true,
                columns:
                    [
                        ...
                        {
                            data: 'kalKode', searchable: false, orderable: false,
            ...
            // untuk dropdown kota dan Kategori juga auto selected berdasarkan data yang diambil dari akun
            window.edit = function (element) {
                var username = element.data('id');
                var baseUrl = '<?= base_url('master_kategori/edit_kategori') ?>';
                $.ajax({
                    url: '<?= base_url('master_kategori/get_data_by_id') ?>',
                    type: 'POST',
                    data: { kalKode: username },
                    dataType: 'json',
                    success: function (data) {
                        // tambahkan username menjadi parameter di url form action jadi dashboard/edit_akun/${username}
                       
                        
                        $('#kalKodeEdit').val(data.kalKode);
                        $('#kalNamaEdit').val(data.kalNama);
                        $('#kalSingkatanEdit').val(data.kalSingkatan);
            ...
            window.setModalHapus = function (element) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '<?= base_url('master_kategori/hapus_kategori') ?>',
                            type: 'POST',
                            data: { kalKode: username },
